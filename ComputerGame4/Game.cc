...
{
    GameState game_state(LEN_X, std::string(LEN_Y, '.')); //Y Spalten, X Zeilen
...
{
    for (unsigned int i = 0; i < LEN_X; i++) //Konsolen-Ausgabe
    {
...
{
    bool finished = false;
    if (GOAL == player) //operator ist überladen für pair, jeweils beide Pair-Werte werden verglichen
    {
        finished = true;
    }
    return finished;
...
unsigned int random_uint(const unsigned int lower, const unsigned int upper)
{
    ...
    return dist(gen); //sampling random unsigned int out of distribution
}
...
void game()
{
    ...
    {
        obs = random_position(1, LEN_X - 2, 1, LEN_Y - 2);
    }
